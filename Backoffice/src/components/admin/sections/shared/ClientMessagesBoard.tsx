...
import { clientMessages, type ClientMessageThread } from '../../UsersManagement';
import { MessageSquareText, Filter, Search, Send, AlertTriangle, CheckCircle2, Phone } from 'lucide-react';

type ClientMessagesBoardProps = {
  onReply?: (threadId: string, message: string) => void;
};

const priorityBadges: Record<ClientMessageThread['priority'], { label: string; className: string }> = {
...
export function ClientMessagesBoard({ onReply }: ClientMessagesBoardProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | ClientMessageThread['status']>('all');
  const [selectedThreadId, setSelectedThreadId] = useState<string | null>(clientMessages[0]?.id ?? null);
  const [draftReply, setDraftReply] = useState('');

  const filteredThreads = useMemo(() => {
    return clientMessages.filter((thread) => {
      const matchesSearch =
  ...
  const stats = useMemo(() => {
    const open = clientMessages.filter((message) => message.status === 'open').length;
    const waiting = clientMessages.filter((message) => message.status === 'waiting').length;
    const resolved = clientMessages.filter((message) => message.status === 'resolved').length;
    return { open, waiting, resolved, total: clientMessages.length };
  }, []);
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative flex-1">
              <Input
                placeholder="Rechercher par client, sujet, ville..."
                value={searchQuery}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Avatar className="size-10">
                          <AvatarImage src={thread.avatarUrl} alt={thread.clientName} />
                          <AvatarFallback>
                            {thread.clientName
                              .split(' ')
                              .map((part) => part[0])
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <Badge className={`${priority.className} rounded-full text-[10px]`}>{priority.label}</Badge>
                ...
                <>
                  <div className="p-5 border-b border-gray-100 flex flex-col gap-3">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <Badge className={`${priorityBadges[selectedThread.priority].className} rounded-full`}>
                        {priorityBadges[selectedThread.priority].label}
                      </Badge>
                    </div>
                    <div className="flex flex-wrap gap-3 text-sm text-gray-500">
                      <span>{selectedThread.clientName}</span>
                      <span className="text-gray-300">•</span>
                      <span>{selectedThread.clientUsername}</span>
                      <span className="text-gray-300">•</span>
                  ...
                  <div className="border-t border-gray-100 p-5 space-y-3 bg-white">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button className="rounded-xl gap-2" onClick={handleSendReply} disabled={!draftReply.trim()}>
                          Envoyer
