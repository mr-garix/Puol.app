...
import { hostMessages, type HostMessageThread } from '../../UsersManagement';
import {
  MessageSquareText,
  Filter,
  Search,
...
type HostMessagesBoardProps = {
  onReply?: (threadId: string, message: string) => void;
};

const priorityBadges: Record<HostMessageThread['priority'], { label: string; className: string }> = {
...
export function HostMessagesBoard({ onReply }: HostMessagesBoardProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | HostMessageThread['status']>('all');
  const [selectedThreadId, setSelectedThreadId] = useState<string | null>(hostMessages[0]?.id ?? null);
  const [draftReply, setDraftReply] = useState('');

  const filteredThreads = useMemo(() => {
    return hostMessages.filter((thread) => {
      const matchesSearch =
  ...
  const stats = useMemo(() => {
    const open = hostMessages.filter((message) => message.status === 'open').length;
    const waiting = hostMessages.filter((message) => message.status === 'waiting').length;
    const resolved = hostMessages.filter((message) => message.status === 'resolved').length;
    return { open, waiting, resolved, total: hostMessages.length };
  }, []);
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative flex-1">
              <Input
                placeholder="Rechercher par hôte, sujet, ville..."
                value={searchQuery}
