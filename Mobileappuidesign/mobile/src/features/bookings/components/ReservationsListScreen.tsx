...
const getStatusConfig = (status: ReservationRecord['status']) => {
  switch (status) {
    ...
    case 'completed':
      return { label: 'Terminée', backgroundColor: 'rgba(59,130,246,0.1)', color: '#3B82F6' };
    default:
...
export const ReservationsListScreen: React.FC<ReservationsListScreenProps> = ({ onBack, onReservationPress }) => {
  ...
  const showLoadingState = isLoading && listData.length === 0;
  const showErrorState = Boolean(error) && listData.length === 0;
    ...
    <View style={styles.container}>
      ...
      {showLoadingState ? (
        <View style={styles.feedbackState}>
          ...
          <Text style={styles.feedbackTitle}>Chargement de vos réservations…</Text>
        </View>
      ) : showErrorState ? (
        <View style={styles.feedbackState}>
          ...
          <Text style={styles.emptyTitle}>Impossible de charger vos réservations</Text>
          <Text style={styles.emptyDescription}>
            Vérifiez votre connexion et réessayez.
          </Text>
          <TouchableOpacity style={styles.retryButton} onPress={refreshReservations} activeOpacity={0.9}>
