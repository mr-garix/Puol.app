...
export const ReservationModal: React.FC<ReservationModalProps> = ({
  ...
  unavailableDatesSet.forEach((date) => {
    const status = unavailableDateStatuses?.[date] ?? 'blocked';
    const colors =
      status === 'reserved'
        ? { dotColor: '#3B82F6', textColor: '#3B82F6' }
        : { dotColor: '#F87171', textColor: '#B91C1C' };
  ...
  if (checkInDate) {
    markedDates[formatDateForCalendar(checkInDate)] = {
      selected: step === 'check-in',
  ...
  if (checkOutDate) {
    markedDates[formatDateForCalendar(checkOutDate)] = {
      selected: step === 'check-out',
      selectedColor: '#2ECC71',
      marked: true,
      dotColor: '#2ECC71',
    };
